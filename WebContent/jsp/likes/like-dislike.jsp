...
String title="";
String titleUrl="";
if(request.getParameter("contentId")!=null && request.getParameter("contentId").length()>0)
...
if(request.getParameter("titleUrl")!=null)
	titleUrl = request.getParameter("titleUrl");


if(request.getParameter("galleryId")!=null && request.getParameter("galleryId").length()>0) 
...
}catch(Exception exception){ upcount=0;downcount=0;totalcount=0;}
%>
<script>
//alert("aa--");
...
<%if(title!=null && title.equals("no")) {%>
    <div class="lk_icon"><a onClick="javascript:changeRating_story('<%=contentId%>','<%=contentType%>','1')"><img src="http://media2.intoday.in/indiatoday/megacritic/images/lke.png" alt="Like" /></a></div>
         <div class="lk_des"><span class="likecount" id="upcount<%=contentId%>">  <%=upcount%>  </span>   Want to watch</div> 
...
<%}else{ %>
    ...
    <input type="hidden" id="likecountdata" value="<%=upcount%>"/>
               ...
               <div class="content_panel">
                <h3><a href="<%=MegacriticCommonConstants.SITE_URL+titleUrl%>"><%=title%></a> </h3>
